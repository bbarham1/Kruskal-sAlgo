...
class Graph310 implements Graph<GraphNode, GraphEdge>, UndirectedGraph<GraphNode, GraphEdge> {
	...
	public Collection<GraphEdge> getEdges() {
		...
		for (Map310<GraphNode, GraphEdge> nodeEdges : storage.values()) {
			edges.addAll(nodeEdges.values());
		}
		return edges;
	...
	public int getEdgeCount() {
		...
		return getEdges().size(); // default return, remove or change as needed
	...
	public boolean containsVertex(GraphNode vertex) {
		...
		return storage.containsKey(vertex); // default return, remove or change as needed
	}
	...
	public int getNeighborCount(GraphNode vertex) {
		...
		if (containsVertex(vertex) && vertex != null) {
			return storage.get(vertex).size();
		}
	...
	public boolean isIncident(GraphNode vertex, GraphEdge edge) {
		if (containsVertex(vertex)) {
			return storage.get(vertex).containsValue(edge);
		}
	 ...
	 * @return the endpoints (incident vertices) of edge or null if edge is not
	 *         present
	 */
	public Pair<GraphNode> getEndpoints(GraphEdge edge) {

		// NOTE: the two end points must be sorted in ascending order of their IDs.
		for (GraphNode v1 : storage.keySet()) {
			Map310<GraphNode, GraphEdge> vertexEdges = storage.get(v1);
			for (GraphNode v2 : vertexEdges.keySet()) {
				if (vertexEdges.get(v2).equals(edge)) {
	 ...
	 * to addEdge(e, new Pair GraphNode (v1, v2)). If this graph does not contain
	...
	public boolean addEdge(GraphEdge e, GraphNode v1, GraphNode v2) {
		...
		if (containsVertex(v1) && containsVertex(v2)) {
			if (findEdge(v1, v2) == null && findEdge(v2, v1) == null) {
	...
	public boolean removeEdge(GraphEdge edge) {
		...
		;
		if (getEndpoints(edge) != null) {
			storage.get(getEndpoints(edge).getFirst()).remove(getEndpoints(edge).getSecond());
			storage.get(getEndpoints(edge).getSecond()).remove(getEndpoints(edge).getFirst());
			return true;
		}
		return false; // default return, remove or change as needed
	}
	...
	public boolean removeVertex(GraphNode vertex) {
		...
		if (vertex == null || !containsVertex(vertex)) {
			...
			for (GraphNode neigh : getNeighbors(vertex)) {
				removeEdge(findEdge(vertex, neigh));
			}
	...
	/**
	 * Report a set of graph nodes that are reachable from the given vertex. Do not
	 * include vertex itself in the set.
	 * <p>
	 * No particular order is required (hence a set).
	 * </p>
	 * 
	 * @param vertex node
	 * @return a set of graph nodes that are reachable from vertex, or an empty set
	 *         if none return null if vertex is not present.
	 */
	public Set310<GraphNode> reachableSet(GraphNode vertex) {
		...
		if (containsVertex(vertex)) {
			Set310<GraphNode> visited = new Set310<>();

			visited.add(vertex);
			for (GraphNode neighbor : getNeighbors(vertex)) {
				if (!visited.contains(neighbor)) {
					reachableSet2(neighbor, visited);
				}
			}
	...
	/**
	 * private method to finde reachable set helper and add vertex.
	 * 
	 * @param vertex  node
	 * @param visited set graphnode
	 */
	private void reachableSet2(GraphNode vertex, Set310<GraphNode> visited) {
		visited.add(vertex);
		for (GraphNode neighbor : getNeighbors(vertex)) {
			if (!visited.contains(neighbor)) {
				reachableSet2(neighbor, visited);
	...
	public static void main(String[] args) {
		...
		/*
		 * for (GraphNode n: reachable){ System.out.print(n.getId()+" "); }
		 * System.out.println();
		 */
	 ...
	 * 
	 * @param directedEdge directed edge in graph
	 ...
	 * </ul>
	 * 
	 * @param edge     reprents edge in graph
	 * @param vertices represents vertex in graph
	 * @return true if the add is successful, and false otherwise
	 ...
	 * @param vertices vertex in graph
